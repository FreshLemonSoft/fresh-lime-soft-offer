<template>
  ...
  <div class="offer content">
      ...
      <p class="text">Наша последняя встреча и твое успешное техническое собеседование многое поставили на свое место и мы поняли, что именно такого человека не хватает в нашей команде в <a href="https://freshlimesoft.com" target="_blanc">Fresh Lime Soft</a> на должности {{NEW_INPUTS.rank || '............'}}!</p>
      <p class="text" v-if="NEW_INPUTS.offerType === 'trainee'">На время стажировки, которая длится {{NEW_INPUTS.traineePeriod || '............'}} и испытательного срока, который длится {{NEW_INPUTS.qualifyingPeriod || '............'}} мы заключаем договор подряда и предлагаем з/п в размере {{NEW_INPUTS.traineeSalary || '............'}} на стажировке и {{NEW_INPUTS.qualifyingSalary || '............'}} на испытательном сроке. При этом, если ты сразу попадаешь на проект и будешь показывать хорошие результаты, то сроки могут сократиться. После заключаем контракт на год с з/п в размере {{NEW_INPUTS.mainSalary || '............'}}.</p>
      <p class="text" v-else>На время испытательного срока, который длится {{NEW_INPUTS.qualifyingPeriod || '............'}} мы заключаем договор подряда и предлагаем з/п в размере {{NEW_INPUTS.qualifyingSalary || '............'}}. При этом, если ты сразу попадаешь на проект и будешь показывать хорошие результаты, то сроки могут сократиться. После заключаем контракт на год с з/п в размере {{NEW_INPUTS.mainSalary || '............'}}.</p>
      <p class="text">Еще раз напомним условия и требования из вакансии: <a :href="NEW_INPUTS.vacancyLink ">{{NEW_INPUTS.vacancyLink || '         '}}</a></p>
